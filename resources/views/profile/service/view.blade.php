@extends('profile.app')
@section('title', 'assigned service lists')

@section('main-content')
    <div class="container">
        <h3 class="profile-menu-title">List of Assign services </h3>
        <div class="line-margin"></div>
        <div class="row">
            @foreach($services as $service)
                <div class="col-lg-4">
                    <div class="service-lists">
                        <a href="{{route('subcategory.show',[$service->servicerequest->service->slug])}}" class="service-name">
                            @if(file_exists('storage/'.$service->servicerequest->service->thumbnail) && $service->servicerequest->service->thumbnail !='')
                                <img src="{{asset('storage/'.$service->servicerequest->service->thumbnail)}}" alt="{{$service->servicerequest->service->title}}">
                            @endif
                            {{--                            <div class="overlay">--}}
                        ...
                        <div class="service-details">
                            <div class="service-content">
                                <p>Title: <span>{{$service->servicerequest->service->title}}</span></p>
                                <p>Rate: <span>{{$service->servicerequest->service->rate}} / {{$service->servicerequest->service->rate_type}}</span></p>
                            </div>
                            <div class="service-brnt auth-order-service">
                                ...
                                <a href="{{route('assign.service.show',[$service->id])}}" class="btn  edit-btn">View Details</a>
                            </div>
                        </div>
                    </div>
